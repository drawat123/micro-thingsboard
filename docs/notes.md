...
Inversion of Control (IoC) is a core architectural design principle in the Spring Framework where the control of object creation, configuration, and lifecycle management is transferred from the application code to the framework itself.

### Spring vs. Spring Boot — Settle This Now

This trips up beginners constantly. Quick clarification:
...
### How Spring Instantiates `BackendApplication`
...
Spring uses **Java Reflection** to inspect the class and create an object at runtime. Internally (simplified), Spring performs something similar to::

```java
public static void run(Class<?> primarySource, String... args) {
    // Inspect the class
    Constructor<?> constructor =
            primarySource.getDeclaredConstructor();
...
This same mechanism is used for all Spring beans. Spring scans for annotations such as `@Service`, `@Component`, and `@Repository`, inspects their constructors, creates the objects, injects required dependencies, and stores them in the Application Context.

**Key takeaway:** We declare classes and dependencies, while Spring uses reflection to instantiate and wire them together. This is the core idea behind Dependency Injection (DI) and Inversion of Control (IoC).
...
Reach for the builder pattern. This is Effective Java, Item 2, in action. You'll see it in Java's HttpRequest.Builder, Spring's WebClient.Builder, Lombok's @Builder, the Stream API's Stream.Builder, and now Protobuf and Spring Integration.


### Design Things

#### Single Responsibility Principle
The Mental Model: Single Responsibility, Made Physical
There's a principle called Single Responsibility — a class should have one reason to change.

#### Configuration and Service files
Configuration files describe wiring. Service classes hold logic. Keeping them separate means:
...
#### The Stereotype Annotations: @Component vs @Service vs @Configuration

A quick clarification, because they all do roughly the same thing but signal different intent.
...
When you mark a class @Service, you're telling the next developer: "this is where business behavior lives, not configuration, not HTTP handling, not database queries." Some tools (like Spring's AOP modules) treat them differently — @Repository adds automatic exception translation, for example.
Architect's heuristic: Use the most specific annotation that fits. If it's business logic, @Service. If it's data access, @Repository. If it's wiring, @Configuration. Generic @Component is a fallback when nothing else fits.
...
@Autowired
public void setOther(OtherService other) { this.other = other; }
}
